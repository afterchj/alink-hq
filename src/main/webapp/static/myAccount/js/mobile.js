...
$(function () {
    ...
    $("#codeSubmit").click(function () {
        ...
        if (mobileFlag){
            //手机号为空
            $('#phone').prev('.verify').removeClass('active').text('');
            $('#phone').prev('.verify').addClass('active').text("请输入手机号");
        }else if(!hintFlag){
            //手机号格式有误
            $('#phone').prev('.verify').removeClass('active').text('');
            $('#phone').prev('.verify').addClass('active').text(mobileHint);
        }else {
            $.ajax({
                type:"POST",
                ...
                success:function (msg) {
                    ...
                    if (msg.info=="isBinding"){
                        $('#phone').prev('.verify').removeClass('active').text('');
                        $('#phone').prev('.verify').addClass('active').text("该手机号已绑定");
...
//     var account = $(".account").val();
    ...
    if (mobileFlag){
        ...
        //手机号格式有误
        $('#phone').prev('.verify').removeClass('active').text('');
        $('#phone').prev('.verify').addClass('active').text("请输入正确的手机号");
    }
    if (codeFlag){
        //激活码为空
        $('#code').prev('.verify').removeClass('active').text('');
        $('#code').prev('.verify').addClass('active').text("请输入验证码");
    }
    if (!mobileFlag&&!codeFlag&&hintFlag){
        ...
        if ($.trim(code).length!=6){
            //激活码不正确
            $('#code').prev('.verify').removeClass('active').text('');
            $('#code').prev('.verify').addClass('active').text("验证码不正确");
        }else {
            $('#phone').prev('.verify').removeClass('active').text('');
            $.ajax({
                type:"POST",
